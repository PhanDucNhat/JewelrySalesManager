                ...
                <div class="form_tt border">
                    ...
                    <div class="container">
                        ...
                        <div class="main__content main__edit shadow bg-white rounded">
                            <form method="post">
                                <div class="box-data border rounded">
                                    <p>Hoạt động trong tháng <?php echo date('m/Y', strtotime($date)); ?> </p>
                                    <ul><a href="#"><i class="fas fa-sync-alt" aria-hidden="true"></i></a></ul>
                                </div>
                                <div class="form_tt border">
                                    <div class="row">
                                        <div class="col">
                                            <div class="p-3 border bg-light rounded">
                                                <div class="row_order title_order d-flex align-items-center">
                                                    <i class="fas fa-calculator" aria-hidden="true"> Thống kê</i>
                                                    <span><?php echo mysqli_fetch_row(mysqli_query($con, "SELECT COUNT(*) FROM dathang WHERE DATE_FORMAT(ThoiGianDat, '%Y-%m') = '$month'"))[0]; ?></span>
                                                </div>
                                                <div class="row_order row_tt">
                                                    <span>Số lượng đồng hồ bán được</span>
                                                    <span><?php
                                                        $query = "SELECT COUNT(*) FROM ctdathang
                                                                JOIN dathang ON ctdathang.MaPhieuDat = dathang.MaPhieuDat
                                                                WHERE DATE_FORMAT(dathang.ThoiGianDat, '%Y-%m') = '$month' AND dathang.TrangThai = 'hoàn thành'";
                                                        echo mysqli_fetch_row(mysqli_query($con, $query))[0];
                                                    ?></span>
                                                </div>
                                                <div class="row_order row_tt">
                                                    <span>Doanh thu</span>
                                                    <span><?php
                                                        $query = "SELECT SUM(ctdathang.SoLuong * ctdathang.Gia) FROM ctdathang
                                                                JOIN dathang ON ctdathang.MaPhieuDat = dathang.MaPhieuDat
                                                                WHERE DATE_FORMAT(dathang.ThoiGianDat, '%Y-%m') = '$month' AND dathang.TrangThai = 'hoàn thành'";
                                                        echo number_format(mysqli_fetch_row(mysqli_query($con, $query))[0], 0, ".", ",") . " đ";
                                                    ?></span>
                                                </div>
                                            </div>
                                        </div>
